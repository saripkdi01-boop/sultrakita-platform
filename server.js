...
app.post('/api/auth/verify-otp', otpRateLimit, async (req, res, next) => {
  try {
    ...
    ok(res, { token, user: { id: user.id, name: user.name, phone: user.phone, role: user.role, district: user.district, phone_verified: true, verification_status: user.verification_status || 'unverified' } });
  } catch (error) { next(error); }
});
...
app.get('/api/admin/feature-flags', requireRole('admin'), adminOnly, async (_req, res, next) => { try { ok(res, await query('SELECT key, enabled, config, updated_at FROM feature_flags ORDER BY key')); } catch (error) { next(error); } });
...
app.patch('/api/admin/verifications/:id', requireRole('admin'), adminOnly, async (req, res, next) => { try { const status = req.body?.status; if (!['approved','rejected','pending'].includes(status)) return fail(res, 422, 'Status verifikasi tidak valid'); const rows = await query('SELECT user_id FROM seller_verifications WHERE id = ?', [Number(req.params.id)]); if (!rows.length) return fail(res, 404, 'Pengajuan verifikasi tidak ditemukan'); await run('UPDATE seller_verifications SET status = ?, note = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?', [status, req.body.note || null, Number(req.params.id)]); await run('UPDATE users SET verification_status = ?, verification_note = ? WHERE id = ?', [status, req.body.note || null, rows[0].user_id]); ok(res, { id:Number(req.params.id), status }); } catch (error) { next(error); } });
app.get('/api/admin/reports', requireRole('admin'), adminOnly, async (req, res, next) => { try { ok(res, await query("SELECT r.*, l.title FROM reports r LEFT JOIN listings l ON l.id = r.listing_id WHERE r.status = ? ORDER BY r.created_at DESC", [req.query.status || 'open'])); } catch (error) { next(error); } });
app.patch('/api/admin/reports/:id', requireRole('admin'), adminOnly, async (req, res, next) => { try { const status = req.body?.status; if (!['open','reviewing','resolved','rejected'].includes(status)) return fail(res, 422, 'Status laporan tidak valid'); await run('UPDATE reports SET status = ? WHERE id = ?', [status, Number(req.params.id)]); ok(res, { id:Number(req.params.id), status }); } catch (error) { next(error); } });

app.post('/api/conversations', async (req, res, next) => { try { const { listing_id, buyer_id, seller_id } = req.body || {}; if (!positiveInt(buyer_id) || !positiveInt(seller_id)) return fail(res, 422, 'buyer_id dan seller_id wajib valid'); if (!req.user) return fail(res, 401, 'Autentikasi diperlukan'); if (Number(req.user.id) !== Number(buyer_id) && req.user.role !== 'admin') return fail(res, 403, 'Identitas buyer tidak sesuai session'); if (positiveInt(listing_id)) { const [listing] = await query('SELECT seller_id FROM listings WHERE id = ?', [Number(listing_id)]); if (!listing) return fail(res, 404, 'Listing tidak ditemukan'); if (Number(listing.seller_id) !== Number(seller_id) && req.user.role !== 'admin') return fail(res, 403, 'Seller tidak sesuai dengan listing'); } const existing = await query('SELECT * FROM conversations WHERE listing_id IS NOT DISTINCT FROM ? AND buyer_id = ? AND seller_id = ?', [listing_id || null, Number(buyer_id), Number(seller_id)]); if (existing.length) return ok(res, existing[0]); const created = await run('INSERT INTO conversations (listing_id, buyer_id, seller_id) VALUES (?, ?, ?)', [listing_id || null, Number(buyer_id), Number(seller_id)]); const [conversation] = await query('SELECT * FROM conversations WHERE id = ?', [created.id]); res.status(201); ok(res, conversation); } catch (error) { next(error); } });
app.use('/api/conversations/:id/messages', async (req, res, next) => { if (!positiveInt(req.params.id)) return fail(res, 400, 'ID percakapan tidak valid'); if (req.method === 'POST' && (!positiveInt(req.body?.sender_id) || !req.body?.body || String(req.body.body).trim().length < 1 || String(req.body.body).trim().length > 2000)) return next(); if (!req.user) return fail(res, 401, 'Autentikasi diperlukan'); const [member] = await query('SELECT id FROM conversations WHERE id = ? AND (buyer_id = ? OR seller_id = ?)', [Number(req.params.id), Number(req.user.id), Number(req.user.id)]); if (!member && req.user.role !== 'admin') return fail(res, 403, 'Akses tidak diizinkan'); next(); });
app.use('/api/conversations/:id/stream', async (req, res, next) => { if (!positiveInt(req.params.id)) return fail(res, 400, 'ID percakapan tidak valid'); if (!req.user) return fail(res, 401, 'Autentikasi diperlukan'); const [member] = await query('SELECT id FROM conversations WHERE id = ? AND (buyer_id = ? OR seller_id = ?)', [Number(req.params.id), Number(req.user.id), Number(req.user.id)]); if (!member && req.user.role !== 'admin') return fail(res, 403, 'Akses tidak diizinkan'); next(); });
app.get('/api/conversations/:id/messages', async (req, res, next) => { try { if (!positiveInt(req.params.id)) return fail(res, 400, 'ID percakapan tidak valid'); if (!req.user) return fail(res, 401, 'Autentikasi diperlukan'); const [member] = await query('SELECT id FROM conversations WHERE id = ? AND (buyer_id = ? OR seller_id = ?)', [Number(req.params.id), Number(req.user.id), Number(req.user.id)]); if (!member && req.user.role !== 'admin') return fail(res, 403, 'Akses tidak diizinkan'); ok(res, await query('SELECT m.*, u.name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.conversation_id = ? ORDER BY m.created_at ASC', [Number(req.params.id)])); } catch (error) { next(error); } });
app.post('/api/conversations/:id/messages', async (req, res, next) => { try { const { sender_id, body } = req.body || {}; if (!positiveInt(sender_id) || !body || body.trim().length < 1 || body.trim().length > 2000) return fail(res, 422, 'sender_id dan pesan belum valid'); if (!req.user) return fail(res, 401, 'Autentikasi diperlukan'); if (Number(req.user.id) !== Number(sender_id) && req.user.role !== 'admin') return fail(res, 403, 'Identitas pengirim tidak sesuai session'); const conversations = await query('SELECT c.*, l.title AS listing_title, seller.name AS seller_name, seller.phone AS seller_phone FROM conversations c LEFT JOIN listings l ON l.id = c.listing_id LEFT JOIN users seller ON seller.id = c.seller_id WHERE c.id = ?', [Number(req.params.id)]); if (!conversations.length) return fail(res, 404, 'Percakapan tidak ditemukan'); const conversation = conversations[0]; if (conversation.status !== 'open') return fail(res, 409, 'Percakapan sudah ditutup'); const created = await run('INSERT INTO messages (conversation_id, sender_id, body) VALUES (?, ?, ?)', [Number(req.params.id), Number(req.user.id), body.trim()]); await run('UPDATE conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?', [Number(req.params.id)]); const [message] = await query('SELECT m.*, u.name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.id = ?', [created.id]); if (Number(sender_id) === Number(conversation.buyer_id)) void notifySellerWhatsApp({ phone:conversation.seller_phone, sellerName:conversation.seller_name, listingTitle:conversation.listing_title, senderName:message.sender_name, message:message.body, channel:'pesan pembeli' }); res.status(201); ok(res, message); } catch (error) { next(error); } });
...
app.post('/api/listings', async (req, res, next) => {
  try {
    ...
    if (!['seller', 'admin'].includes(req.user.role)) return fail(res, 403, 'Akun belum memiliki hak seller');
...
app.put('/api/listings/:id', async (req, res, next) => {
  try {
    ...
    if (!owned.length && req.user.role !== 'admin') return fail(res, 403, 'Akses tidak diizinkan');
...
app.post('/api/favorites', async (req, res, next) => {
  try {
    ...
    const listing = await query("SELECT id FROM listings WHERE id = ? AND status = 'active'", [Number(listing_id)]); if (!listing.length) return fail(res, 404, 'Listing tidak ditemukan'); await run('INSERT OR IGNORE INTO favorites (user_id, listing_id) VALUES (?, ?)', [Number(req.user.id), Number(listing_id)]);
    ok(res, { user_id: Number(req.user.id), listing_id: Number(listing_id), favorited: true });
  } catch (error) { next(error); }
});
