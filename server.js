...
app.post('/api/conversations/:id/messages', async (req, res, next) => { try { const { sender_id, body } = req.body || {}; if (!positiveInt(sender_id) || !body || body.trim().length < 1 || body.trim().length > 2000) return fail(res, 422, 'sender_id dan pesan belum valid'); const conversations = await query('SELECT id FROM conversations WHERE id = ?', [Number(req.params.id)]); if (!conversations.length) return fail(res, 404, 'Percakapan tidak ditemukan'); const created = await run('INSERT INTO messages (conversation_id, sender_id, body) VALUES (?, ?, ?)', [Number(req.params.id), Number(sender_id), body.trim()]); await run('UPDATE conversations SET updated_at = CURRENT_TIMESTAMP WHERE id = ?', [Number(req.params.id)]); const [message] = await query('SELECT m.*, u.name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.id = ?', [created.id]); res.status(201); ok(res, message); } catch (error) { next(error); } });
