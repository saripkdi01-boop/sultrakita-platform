...
app.post('/api/listings', async (req, res, next) => {
  try {
    ...
    const result = await run(`INSERT INTO listings (seller_id, category_id, title, description, price, condition, district, city, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req.user.id, Number(category_id), title.trim(), description.trim(), Number(price), condition, district, city, image_url]);
...
app.put('/api/listings/:id', async (req, res, next) => {
  try {
    if (!positiveInt(req.params.id)) return fail(res, 400, 'ID listing tidak valid');
    if (!req.user) return fail(res, 401, 'Autentikasi diperlukan');
    const { title, description, price, category_id, condition = 'new', district = 'Kendari', city = 'Kendari', image_url = null } = req.body || {};
    ...
    await run('UPDATE listings SET title = ?, description = ?, price = ?, category_id = ?, condition = ?, district = ?, city = ?, image_url = ? WHERE id = ?', [title.trim(), description.trim(), Number(price), Number(category_id), condition, district, city, image_url, Number(req.params.id)]);
...
app.post('/api/favorites', async (req, res, next) => {
  try {
    ...
    await run('INSERT OR IGNORE INTO favorites (user_id, listing_id) VALUES (?, ?)', [Number(req.user.id), Number(listing_id)]);
