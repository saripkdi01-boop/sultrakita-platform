...
setInterval(() => { run('DELETE FROM analytics_events WHERE created_at < datetime(\'now\', ?)', [`-${retentionDays} days`]).catch(() => {}); run('DELETE FROM sessions WHERE expires_at <= ?', [Date.now()]).catch(() => {}); run('DELETE FROM otp_challenges WHERE expires_at < ?', [Date.now()]).catch(() => {}); }, 24 * 60 * 60 * 1000).unref();
...
app.get('/api/analytics/summary', requireRole('admin'), adminOnly, async (req, res, next) => { try { const days = Math.min(90, Math.max(1, Number(req.query.days) || 7)); const [totals] = await query("SELECT COUNT(*) AS events, SUM(CASE WHEN event_name = 'page_view' THEN 1 ELSE 0 END) AS page_views, SUM(CASE WHEN event_name = 'listing_view' THEN 1 ELSE 0 END) AS listing_views, SUM(CASE WHEN event_name = 'search' THEN 1 ELSE 0 END) AS searches, SUM(CASE WHEN event_name = 'listing_contact' THEN 1 ELSE 0 END) AS contacts FROM analytics_events WHERE created_at >= datetime('now', ?)", [`-${days} days`]); const topListings = await query("SELECT l.id, l.title, l.views, COUNT(a.id) AS tracked_views FROM listings l LEFT JOIN analytics_events a ON a.listing_id = l.id AND a.event_name = 'listing_view' AND a.created_at >= datetime('now', ?) GROUP BY l.id ORDER BY tracked_views DESC, l.views DESC LIMIT 10", [`-${days} days`]); const daily = await query("SELECT date(created_at) AS date, COUNT(*) AS events, SUM(CASE WHEN event_name = 'page_view' THEN 1 ELSE 0 END) AS page_views, SUM(CASE WHEN event_name = 'listing_view' THEN 1 ELSE 0 END) AS listing_views FROM analytics_events WHERE created_at >= datetime('now', ?) GROUP BY date(created_at) ORDER BY date ASC", [`-${days} days`]); ok(res, { days, totals, top_listings: topListings, daily }); } catch (error) { next(error); } });
