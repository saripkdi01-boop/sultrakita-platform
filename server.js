...
const notifySellerWhatsApp = async ({ phone, sellerName, listingTitle, senderName, message, channel='pertanyaan' }) => { if (!phone) return; const text = `Halo ${sellerName || 'Penjual'}, ada ${channel} baru di SultraKita.\n\nListing: ${listingTitle || '-'}\nDari: ${senderName || 'Pengguna SultraKita'}\nPesan: ${String(message || '').slice(0, 900)}\n\nBuka platform SultraKita untuk membalas.`; try { await sendWhatsAppText(phone, text); } catch (error) { console.error('[whatsapp-notification]', error.message); } };
