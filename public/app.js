...
function track(event_name, details = {}) { const payload = JSON.stringify({ event_name, path: location.pathname + location.search, referrer: document.referrer || null, ...details }); if (navigator.sendBeacon) navigator.sendBeacon('/api/analytics/track', new Blob([payload], { type:'application/json' })); else fetch('/api/analytics/track', { method:'POST', headers:{ 'content-type':'application/json' }, body:payload, keepalive:true }).catch(() => {}); }
...
$('#search-button').addEventListener('click', () => { track('search', { category_slug: $('#category-filter')?.value || null, district: $('#district').value || null }); loadListings(); }); $('#search').addEventListener('keydown', event => { if (event.key === 'Enter') loadListings(); }); $('#district').addEventListener('change', () => loadListings()); $('#sort').addEventListener('change', () => loadListings()); $('#open-sell').addEventListener('click', openDialog); $('#open-sell-cta').addEventListener('click', openDialog);
