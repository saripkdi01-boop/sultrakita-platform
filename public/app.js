...
function setupTheme() { const dark = localStorage.getItem('sultra-dark') === 'true'; if (dark) document.body.classList.add('dark'); const button = $('#theme-toggle'); const sync = () => { const enabled = document.body.classList.contains('dark'); button?.setAttribute('aria-pressed', String(enabled)); button?.setAttribute('aria-label', enabled ? 'Aktifkan mode terang' : 'Aktifkan mode gelap'); }; button?.addEventListener('click', () => { document.body.classList.toggle('dark'); localStorage.setItem('sultra-dark', String(document.body.classList.contains('dark'))); sync(); }); sync(); }

function setupExperiencePolish() {
  const search = $('#search');
  ...
  const syncNetwork = () => { if (!connection) return; const offline = !navigator.onLine; connection.hidden = !offline; connection.textContent = offline ? 'Anda sedang offline. Data yang sudah dimuat tetap dapat dilihat, dan perubahan akan dicoba kembali saat koneksi pulih.' : ''; document.body.classList.toggle('is-offline', offline); };
  window.addEventListener('online', syncNetwork); window.addEventListener('offline', syncNetwork); syncNetwork();
  const input = document.querySelector('input[name="images"]'); const preview = $('#image-preview');
  input?.addEventListener('change', () => { if (!preview) return; preview.replaceChildren(); const files = [...input.files]; if (files.length > 5) { input.value = ''; toast('Maksimal 5 foto per listing.', 'error'); return; } const invalid = files.find(file => !/^image\/(jpeg|png|webp)$/.test(file.type) || file.size > 4 * 1024 * 1024); if (invalid) { input.value = ''; toast('Gunakan JPG, PNG, atau WEBP maksimal 4 MB per foto.', 'error'); return; } files.forEach(file => { const figure = document.createElement('figure'); figure.className = 'preview-tile'; const image = document.createElement('img'); image.alt = `Pratinjau ${file.name}`; image.loading = 'lazy'; image.src = URL.createObjectURL(file); image.onload = () => URL.revokeObjectURL(image.src); figure.append(image); preview.append(figure); }); });
}
...
async function registerServiceWorker() { if (!('serviceWorker' in navigator)) return; try { await navigator.serviceWorker.register('/sw.js', { scope: '/' }); } catch { /* PWA bersifat opsional; aplikasi tetap berjalan normal. */ } }

async function init() { registerServiceWorker(); setupTheme(); setupExperiencePolish(); refreshFavoriteCount(); bindListingEvents(); setupDiscovery(); setupSellerForm(); setupDonation(); setupAdminDashboard(); $('#suggestion-cta').onclick = () => location.href = 'mailto:halo@sultrakita.id?subject=Saran%20untuk%20SultraKita'; await Promise.all([setupLocations(), loadStats(), loadCategories(), loadExternalListings()]); loadListings(); track('page_view'); }
