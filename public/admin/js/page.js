(() => {
  ...
  async function listings() {
    base('Listings', 'Impor produk dari tautan yang Anda berhak gunakan, koreksi hasil AI lokal, lalu tinjau sebelum tayang.', `<div class="listing-import-layout">
      <section class="listing-import-panel">
        <div class="admin-section-heading"><div><span class="admin-eyebrow">Workflow baru</span><h3>Tambah listing dari tautan</h3><p>Tempel tautan Facebook Marketplace, Tokopedia, OLX, atau Shopee. Server hanya membaca metadata publik yang tersedia.</p></div><span class="admin-badge">AI lokal</span></div>
        <div class="admin-note">Foto dari sumber hanya ditampilkan jika tersedia secara publik. Pastikan Anda memiliki izin memakai foto tersebut atau gunakan foto yang Anda unggah sendiri sebelum publikasi.</div>
        <form id="listing-import-form" class="admin-form-grid">
          <div class="admin-field full"><label for="import-source-url">Tautan sumber produk</label><input id="import-source-url" class="admin-input" type="url" placeholder="https://www.tokopedia.com/..." required><small class="admin-help">HTTPS saja. Domain lain akan ditolak untuk mencegah SSRF dan impor tanpa izin.</small></div>
          <div class="admin-field"><label for="import-category">Kategori</label><select id="import-category" class="admin-select"><option value="">Otomatis dari metadata</option></select></div>
          <div class="admin-field"><label for="import-district">Lokasi</label><select id="import-district" class="admin-select"><option>Kendari</option><option>Mandonga</option><option>Baruga</option><option>Puuwatu</option><option>Kadia</option><option>Wua-Wua</option><option>Poasia</option><option>Abeli</option><option>Kambu</option><option>Kendari Barat</option><option>Nambo</option></select></div>
          <div class="admin-field full"><button class="admin-button" type="submit">Ambil metadata &amp; buat draft</button></div>
        </form>
        <p id="import-status" class="admin-status" aria-live="polite"></p>
        <div id="import-editor" hidden></div>
      </section>
      <aside id="import-preview" class="listing-import-preview"><div class="admin-empty">Pratinjau card akan muncul setelah tautan diproses.</div></aside>
    </div>
    <section class="admin-card import-drafts-card"><div class="admin-section-heading"><div><span class="admin-eyebrow">Review</span><h3>Draft impor Anda</h3><p>Draft tersimpan agar dapat ditinjau kembali sebelum dibuat menjadi listing aktif.</p></div></div><div id="import-drafts" class="admin-empty">Memuat draft…</div></section>
    <div class="admin-toolbar"><div><span class="admin-eyebrow">Native listings</span><h3 class="admin-toolbar-title">Listing yang sudah tayang</h3></div><div class="admin-toolbar-group"><select id="status-filter" class="admin-select" aria-label="Filter status"><option value="">Semua status</option><option>active</option><option>sold</option><option>archived</option></select><button id="reload" class="admin-button secondary">Muat ulang</button></div></div><div id="data-table" class="admin-empty">Memuat…</div>`);
    const categories = await api.request('/api/admin/v2/categories').catch(() => []);
    const categorySelect = document.querySelector('#import-category');
    (Array.isArray(categories) ? categories : []).forEach(category => { const option = document.createElement('option'); option.value = category.id; option.textContent = category.name; categorySelect.appendChild(option); });
    const status = message => { const node = document.querySelector('#import-status'); if (node) { node.textContent = message; node.classList.remove('error'); } };
    const errorStatus = message => { const node = document.querySelector('#import-status'); if (node) { node.textContent = message; node.classList.add('error'); } };
    const preview = draft => {
      const images = (Array.isArray(draft.image_urls) ? draft.image_urls : []).filter(image => /^https:\/\//i.test(image));
      const canPublish = window.AdminAuth?.hasPermission('approve_listings');
      document.querySelector('#import-preview').innerHTML = `<div class="preview-heading"><div><span class="admin-eyebrow">Card preview</span><h3>Siap ditinjau</h3></div><span class="admin-badge">${esc(draft.source_platform)}</span></div><article class="listing-card-preview"><div class="listing-card-preview-media">${images[0] ? `<img src="${esc(images[0])}" alt="${esc(draft.draft_title)}" loading="lazy" width="400" height="300">` : '<span class="preview-placeholder">Foto belum tersedia</span>'}<span class="preview-source">${esc(draft.source_platform)}</span>${images.length > 1 ? `<span class="preview-image-count">▧ ${images.length}</span>` : ''}</div><div class="listing-card-preview-body"><strong>${currency(draft.price)}</strong><h4>${esc(draft.draft_title)}</h4><p>${esc(draft.draft_description)}</p><small>${esc(draft.district)} · ${esc((Array.isArray(categories) ? categories.find(category => Number(category.id) === Number(draft.category_id))?.name : '') || 'Marketplace')}</small></div></article><form id="listing-publish-form" class="admin-form-grid import-editor-form"><div class="admin-field full"><label for="draft-title">Judul card</label><input id="draft-title" class="admin-input" name="title" value="${esc(draft.draft_title)}" required></div><div class="admin-field"><label for="draft-price">Harga (Rp)</label><input id="draft-price" class="admin-input" name="price" type="number" min="0" step="1" value="${Number(draft.price || 0)}" required></div><div class="admin-field"><label for="draft-condition">Kondisi</label><select id="draft-condition" class="admin-select" name="condition"><option value="new" ${draft.condition === 'new' ? 'selected' : ''}>Baru</option><option value="second" ${draft.condition === 'second' ? 'selected' : ''}>Bekas</option></select></div><div class="admin-field"><label for="draft-category">Kategori card</label><select id="draft-category" class="admin-select" name="category_id">${(Array.isArray(categories) ? categories : []).filter(category => Number(category.id) !== 11).map(category => `<option value="${Number(category.id)}" ${Number(category.id) === Number(draft.category_id) ? 'selected' : ''}>${esc(category.name)}</option>`).join('')}</select></div><div class="admin-field"><label for="draft-district">Lokasi card</label><input id="draft-district" class="admin-input" name="district" value="${esc(draft.district || 'Kendari')}" required></div><div class="admin-field full"><label for="draft-description">Deskripsi hasil AI lokal</label><textarea id="draft-description" class="admin-textarea" name="description" required>${esc(draft.draft_description)}</textarea></div><div class="admin-field full"><label>Foto yang dipakai (${images.length}/5)</label><div class="import-image-grid">${images.length ? images.map((image, index) => `<label class="import-image-option"><input type="checkbox" name="image_urls" value="${esc(image)}" checked><img src="${esc(image)}" alt="Foto ${index + 1}" loading="lazy" width="400" height="300"><span>Foto ${index + 1}</span></label>`).join('') : '<span class="admin-help">Tidak ada foto publik dari sumber. Gunakan foto berizin melalui alur upload listing yang tersedia.</span>'}</div></div><div class="admin-field full import-editor-actions"><a href="${esc(draft.source_url)}" target="_blank" rel="noopener noreferrer">Buka sumber asli ↗</a><span></span>${canPublish ? '<button class="admin-button" type="submit">Publikasikan listing</button>' : '<span class="admin-help">Role Anda dapat membuat draft, tetapi tidak memiliki hak publikasi.</span>'}<button class="admin-button ghost" type="button" id="discard-import">Buang draft</button></div></form>`;
      document.querySelector('#listing-publish-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const button = event.currentTarget.querySelector('button[type="submit"]'); if (button) { button.disabled = true; button.textContent = 'Mempublikasikan…'; } try { const result = await api.request(`/api/admin/v2/listing-imports/${Number(draft.id)}/publish`, { method: 'POST', body: JSON.stringify({ title: form.get('title'), description: form.get('description'), price: Number(form.get('price')), category_id: Number(form.get('category_id')), condition: form.get('condition'), district: form.get('district'), image_urls: form.getAll('image_urls') }) }); status(`Listing #${result.listing.id} berhasil dipublikasikan dengan ${result.images.length} foto.`); document.querySelector('#import-editor').hidden = true; document.querySelector('#import-preview').innerHTML = '<div class="admin-empty">Listing tayang. Anda dapat memproses tautan lain.</div>'; await loadDrafts(); await load(document.querySelector('#status-filter').value); } catch (error) { errorStatus(error.message); if (button) { button.disabled = false; button.textContent = 'Publikasikan listing'; } } });
      document.querySelector('#discard-import')?.addEventListener('click', async () => { try { await api.request(`/api/admin/v2/listing-imports/${Number(draft.id)}/discard`, { method: 'POST', body: '{}' }); status('Draft dibuang.'); document.querySelector('#import-preview').innerHTML = '<div class="admin-empty">Draft dibuang.</div>'; await loadDrafts(); } catch (error) { errorStatus(error.message); } });
    };
    const form = document.querySelector('#listing-import-form');
    form.addEventListener('submit', async event => { event.preventDefault(); const button = event.currentTarget.querySelector('button[type="submit"]'); const url = document.querySelector('#import-source-url').value.trim(); if (button) { button.disabled = true; button.textContent = 'Membaca metadata…'; } try { const draft = await api.request('/api/admin/v2/listing-imports/preview', { method: 'POST', body: JSON.stringify({ source_url: url, category_id: document.querySelector('#import-category').value || undefined, district: document.querySelector('#import-district').value }) }); document.querySelector('#import-editor').hidden = false; preview(draft); status(`${draft.source_platform} terbaca. Koreksi field dan foto sebelum publikasi.`); await loadDrafts(); } catch (error) { errorStatus(error.message); } finally { if (button) { button.disabled = false; button.textContent = 'Ambil metadata & buat draft'; } } });
    async function loadDrafts() { const drafts = await api.request('/api/admin/v2/listing-imports?status=draft&limit=20'); const container = document.querySelector('#import-drafts'); if (!drafts.length) { container.innerHTML = '<div class="admin-empty">Belum ada draft impor.</div>'; return; } container.innerHTML = `<div class="import-draft-list">${drafts.map(draft => `<button class="import-draft-row" type="button" data-draft-id="${Number(draft.id)}"><span class="listing-thumb">${esc(String(draft.source_platform).slice(0, 1))}</span><span><strong>${esc(draft.draft_title)}</strong><small>${esc(draft.source_platform)} · ${esc(draft.district)} · ${Number(draft.image_urls?.length || 0)} foto</small></span><b>${currency(draft.price)}</b></button>`).join('')}</div>`; container.querySelectorAll('[data-draft-id]').forEach(button => button.addEventListener('click', async () => { const draft = await api.request(`/api/admin/v2/listing-imports?status=draft&limit=100`); const selected = draft.find(item => Number(item.id) === Number(button.dataset.draftId)); if (selected) { document.querySelector('#import-editor').hidden = false; preview(selected); } })); }
    const load = async filterStatus => { const data = await api.request(`/api/admin/v2/listings?${utils.query({ status: filterStatus, limit: 100 })}`); ui.table('#data-table', [{ key: 'id', label: 'ID' }, { key: 'title', label: 'Judul' }, { key: 'seller_name', label: 'Seller' }, { key: 'price', label: 'Harga', render: row => currency(row.price) }, { key: 'status', label: 'Status', render: row => statusBadge(row.status) }, { key: 'created_at', label: 'Dibuat', render: row => utils.date(row.created_at) }, { key: 'actions', label: 'Aksi', render: row => action('Arsipkan', `/api/admin/v2/listings/${row.id}/status`, { status: 'archived' }) }], data, 'Belum ada listing.'); };
    document.querySelector('#status-filter').addEventListener('change', event => load(event.target.value).catch(error => ui.setStatus(error.message, true))); document.querySelector('#reload').addEventListener('click', () => load(document.querySelector('#status-filter').value).catch(error => ui.setStatus(error.message, true))); await Promise.all([load(''), loadDrafts()]);
  }
