(() => {
  ...
  function shell() {
    ...
    document.querySelectorAll('[data-menu-toggle]').forEach(button => {
      ...
      button.addEventListener('click', () => {
        ...
        document.querySelectorAll('[data-menu-toggle]').forEach(toggle => toggle.setAttribute('aria-expanded', String(open)));
      });
    });
  ...
  function layout(auth) {
    ...
    main.outerHTML = `<div class="admin-shell">
      ...
      <main id="admin-main" class="admin-content" tabindex="-1">
        <header class="admin-topbar">
          <div class="admin-topbar-left"><button class="admin-button secondary admin-mobile-toggle" type="button" data-menu-toggle aria-expanded="false" aria-controls="admin-sidebar" aria-label="Buka navigasi"><span aria-hidden="true">☰</span><span class="admin-mobile-toggle-label">Menu</span></button><a class="admin-topbar-brand" href="/admin/dashboard"><span class="admin-mark">S</span><span>SultraKita<small>Admin</small></span></a><label class="admin-topbar-search"><span aria-hidden="true">⌕</span><input type="search" data-global-search placeholder="Cari pengguna atau listing…" aria-label="Cari pengguna atau listing"></label></div>
          <nav class="admin-center-nav" aria-label="Navigasi cepat"><a href="/admin/dashboard" ${current('dashboard')} data-center-nav="dashboard"><span>⌂</span><small>Beranda</small></a><a href="/admin/analytics.html" ${current('analytics')} data-permission="analytics:view" data-center-nav="analytics"><span>⌁</span><small>Analytics</small></a><a href="/admin/listings.html" ${current('listings')} data-permission="listings:view" data-center-nav="listings"><span>◇</span><small>Listing</small></a><a href="/admin/donations.html" ${current('donations')} data-permission="donations:manage" data-center-nav="donations"><span>♡</span><small>Donasi</small></a><a href="/admin/users.html" ${current('users')} data-permission="users:view" data-center-nav="users"><span>♙</span><small>Pengguna</small></a></nav>
    ...
    <nav class="admin-bottom-nav" aria-label="Navigasi mobile"><a href="/admin/dashboard" ${current('dashboard')}><span>⌂</span><small>Beranda</small></a><a href="/admin/analytics.html" ${current('analytics')} data-permission="analytics:view"><span>⌁</span><small>Analytics</small></a><a href="/admin/listings.html" ${current('listings')} data-permission="listings:view"><span>◇</span><small>Listing</small></a><a href="/admin/donations.html" ${current('donations')} data-permission="donations:manage"><span>♡</span><small>Donasi</small></a><button type="button" data-menu-toggle aria-expanded="false" aria-controls="admin-sidebar" aria-label="Buka menu admin"><span>☰</span><small>Menu</small></button></nav>`;
  }
