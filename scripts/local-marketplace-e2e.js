...
const env = { ...process.env, PORT: String(port), VERCEL: 'true', OTP_DEV_MODE: 'true', CORS_ORIGIN: base, ADMIN_TOKEN: 'local-e2e-admin-token' };
...
async function waitForHealth() { for (let attempt = 0; attempt < 40; attempt += 1) { try { const body = await request('/api/health'); if (body.data?.status === 'healthy') return; } catch {} await sleep(100); } throw new Error(`Server tidak siap. Output: ${output}`); }
...
async function main() {
  ...
  console.log(JSON.stringify({ passed: true, checks: { health: true, otp_session: true, seller_post: true, image_upload: image.data?.length === 1, conversation: true, message_history: history.data?.length === 1, realtime_stream: Boolean(streamResponse), listing_detail: detail.data?.id === listingId }, ids: { listing: listingId, conversation: conversationId }, note: 'Data dibuat di /tmp melalui VERCEL=true dan proses dihentikan setelah test.' }, null, 2));
}
