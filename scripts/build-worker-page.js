...
if(process.argv.includes('--check')){if(worker.slice(start,end+2)!==expected)throw new Error('Worker page drift detected; run npm run sync:worker');console.log('PASS: Worker page matches public frontend assets');}else{const next=worker.slice(0,start)+expected+worker.slice(end+2);fs.writeFileSync(workerPath,next);console.log('PASS: Worker page synchronized');}
