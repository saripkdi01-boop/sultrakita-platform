...
async function main() {
  ...
  const initial = await request('/api/donation/stats');
  if (!initial.body.success) throw new Error('campaign stats failed');
  const created = await request('/api/donations', { method: 'POST', headers: {'content-type':'application/json'}, body: JSON.stringify({amount: 25000, name: 'Smoke Donor'}) });
  ...
  const [campaign] = await query('SELECT current_amount FROM donation_campaigns WHERE id = 1');
  if (settled.response.status !== 200 || replay.response.status !== 200 || Number(campaign.current_amount) !== 25000) throw new Error('settlement or idempotency failed');
