...
const validRoles: UserRole[] = ['buyer', 'seller', 'creator', 'community', 'admin'];
function normalizeProfile(value: unknown): UserProfile {
  ...
  return {
    ...defaultProfile,
    ...source,
    role: validRoles.includes(source.role as UserRole) ? source.role as UserRole : defaultProfile.role,
    interests: Array.isArray(source.interests) ? source.interests.filter((item): item is string => typeof item === 'string') : defaultProfile.interests,
  };
}
