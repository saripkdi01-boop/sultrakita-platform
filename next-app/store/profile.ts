...
export type SettingTab = 'account' | 'privacy' | 'notifications' | 'appearance' | 'seller' | 'help';
...
export const useProfileStore = create<ProfileState>()(persist((set) => ({
  ...
  openSetup: () => set({ menuOpen: false, setupOpen: true }),
  openSettings: (tab = 'account') => set({ menuOpen: false, settingsOpen: true, activeTab: tab }),
  closeOverlays: () => set({ menuOpen: false, setupOpen: false, settingsOpen: false }),
  setActiveTab: (activeTab) => set({ activeTab }),
}), { name: 'sultrakita-profile-storage', partialize: (state) => ({ profile: state.profile }) }));
