...
export default function HomePage() {
  ...
  return <AppLayout onCreate={() => setListingOpen(true)}>
    <main className="platform-shell">
      <section className="platform-hero"><div><span className="eyebrow"><Sparkles size={14}/> Ruang warga Sulawesi Tenggara</span><h1>Belanja lokal,<br/><em>rasa global.</em></h1><p>Temukan produk, jasa, cerita, dan seller terpercaya dari Sultra dalam satu ruang warga.</p><div className="hero-actions"><button className="primary-btn" onClick={() => setActiveTab('market')}><Store size={17}/> Jelajahi marketplace</button><button className="ghost-btn dark" onClick={() => setListingOpen(true)}>Pasang iklan gratis</button></div></div><div className="hero-orb"><span>●</span><strong>SUKI</strong><small>marketplace warga</small></div></section>
      <nav className="platform-tabs" aria-label="Navigasi SultraKita">{[['home', 'Beranda', Users], ['reels', 'Reels', Play], ['market', 'Marketplace', Store], ['groups', 'Grup', Users]].map(([key, label, Icon]) => <button key={key as string} className={activeTab === key ? 'active' : ''} onClick={() => setActiveTab(key as typeof activeTab)}><Icon size={17}/>{label as string}</button>)}</nav>
