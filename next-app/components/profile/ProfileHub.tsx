...
export const categories: { id: SettingTab; label: string; description: string; icon: typeof UserRound }[] = [
...
function ProfileEditForm({ userId }: { userId?: string }) {
  ...
  async function save(event: FormEvent) {
    ...
    setSaving(true);
    const { data, error: updateError } = await supabase.from('profiles').update({ full_name: fullName, display_name: fullName, username: username || null, bio: draft.bio.trim() || null, city: draft.city.trim() || null, district: draft.district.trim() || null, avatar_url: draft.avatar_url.trim() || null }).eq('id', userId).select('full_name,username,bio,city,district,avatar_url').single();
    setSaving(false);
