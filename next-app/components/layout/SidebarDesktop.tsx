...
export function SidebarDesktop() {
  ...
  return <aside className={`desktop-sidebar ${collapsed ? 'collapsed' : ''}`}>
    <div className="sidebar-inner">
      ...
      {!collapsed && <div className="profile-actions"><button aria-label="Segarkan profil"><RefreshCw size={14} /></button><span>{labels.active} di Sultra</span></div>}
      ...
      {!collapsed && <div className="sidebar-footer"><a className="side-upgrade" href="#premium"><Crown size={16} /><span><strong>{labels.premium}</strong><small>Bangun eksistensi publik</small></span></a><div className="side-login"><LogOut size={16} /><span>{labels.logout}</span></div><small>© 2026 SUKI Platforms · SultraKita</small></div>}
