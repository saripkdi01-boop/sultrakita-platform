...
export function SidebarDesktop() {
  ...
  const headline = profile?.bio || profile?.bio || 'Profil belum dilengkapi';
  ...
  return <aside className={`desktop-sidebar ${collapsed ? 'collapsed' : ''}`}>
    <div className="sidebar-inner">
      <div className="profile-mini">{profile?.avatar_url ? <img className="avatar large" src={profile.avatar_url} alt={displayName} /> : <span className="avatar large">{initials}</span>}<div><strong>{displayName}</strong><small>{headline}</small></div></div>
