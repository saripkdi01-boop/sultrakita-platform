...
export async function getProperties(filters: { category?: PropertyInput['category']; regency?: string; district?: string; subdistrict?: string; minPrice?: number; maxPrice?: number; certificateTypes?: string[]; canKpr?: boolean; isLelang?: boolean; takeover?: boolean; featured?: boolean } = {}) { try { const supabase = await getServerSupabase(); let query = supabase.from('properties').select('id,seller_id,category,property_type,condition,furnishing,can_kpr,is_lelang,lelang_type,takeover_status,title,description,slug,price,price_type,is_negotiable,land_area_sqm,building_area_sqm,bedrooms,bathrooms,floors,furnished,ac_available,parking_slots,amenities,district,city,province,address_detail,maps_link,latitude,longitude,nearby_places,certificate_type,shm_status,is_bank_verified,is_admin_verified,images,video_url,virtual_tour_url,status,is_featured,views_count,favorites_count,inquiries_count,created_at').in('status', ['available','rented','sold']).order('is_featured', { ascending: false }).order('created_at', { ascending: false }).limit(60); if (filters.category) query = query.eq('category', filters.category); if (filters.regency) query = query.eq('regency_name', filters.regency); if (filters.district) query = query.eq('district', filters.district); if (filters.subdistrict) query = query.eq('subdistrict_name', filters.subdistrict); if (filters.certificateTypes?.length) query = query.in('certificate_type', filters.certificateTypes); if (filters.canKpr) query = query.eq('can_kpr', true); if (filters.isLelang) query = query.eq('is_lelang', true); if (filters.takeover) query = query.eq('category', 'takeover_kpr'); if (filters.district) query = query.ilike('district', `%${filters.district}%`); if (filters.minPrice !== undefined) query = query.gte('price', filters.minPrice); if (filters.maxPrice !== undefined) query = query.lte('price', filters.maxPrice); if (filters.featured) query = query.eq('is_featured', true); const { data, error } = await query; if (error) throw error; return { ok: true as const, data: data || [] }; } catch (error) { return { ok: false as const, error: errorMessage(error), data: [] }; } }
