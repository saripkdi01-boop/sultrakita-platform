...
function safeUsageStats(input) { const args = objectInput(input); if (Object.keys(args).length > 0) fail('INVALID_INPUT', 'get_mcp_usage_stats accepts no arguments'); return {}; }
...
const READ_TOOL_DEFINITIONS = [
  { name: 'get_mcp_usage_stats', description: 'Get aggregate in-process MCP tool usage counters; no payloads or secrets are returned.', inputSchema: { type: 'object', properties: {}, additionalProperties: false } },
...
function createToolHandler({ get, write = createWriteAdapter(), rateLimit = createRateLimiter(), audit = () => {}, usage = new Map() } = {}) { return async function call(name, input) { rateLimit(name); usage.set(name, (usage.get(name) || 0) + 1); audit({ tool: name, outcome: 'started' }); switch (name) {
  ...
  case 'get_mcp_usage_stats': { safeUsageStats(input); return jsonResult({ tools: Object.fromEntries([...usage.entries()].sort(([a], [b]) => a.localeCompare(b))), total_calls: [...usage.values()].reduce((sum, count) => sum + count, 0) }); }
...
module.exports = { MAX_LIMIT, TOOL_DEFINITIONS, READ_TOOL_DEFINITIONS, WRITE_TOOL_DEFINITIONS, WRITE_ENABLED, createApiAdapter, createWriteAdapter, createMcpServer, createRateLimiter, createToolHandler, redact, safeBusinessId, safeListingSearch, safeListingWrite, safeListingStatus, safeUsageStats };
